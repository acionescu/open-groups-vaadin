...
/**
 * This is going to basically create a tabsheet with all the nested actions from
 * a {@link UserActionList} object
 * 
 * @author adi
 * 
 */
...
//		actionsTabSheet.setSizeFull();
		...
		actionsTabSheet.addListener(new SelectedTabChangeListener() {
			...
			public void selectedTabChange(SelectedTabChangeEvent event) {
				...
				if (entity != null) {
					Deque<String> desiredActionsQueue = entity.getState()
							.getDesiredActionTabsQueue();
					/*
					 * if a desired action exists, it will be set afterwards,
					 * otherwise allow the first action from the list to be
					 * selected by default
					 */
					if (desiredActionsQueue.size() != 0) {
						...
						if (nextAction.equals(ua.getActionName())) {
							...
							/*
							 * if this action does not match with the next
							 * desired action, do nothing
							 */
							return;
						}
					}
				...
				if (ua instanceof UserActionList) {
					...
					if (entity != null) {
						...
						entity.getState().setCurrentTabActionContainer(
								selectedTabContent);
		...
		List<UserAction> actionsList = new ArrayList<UserAction>(ual
				.getActions().values());
...
//			tabContent.setSizeFull();
		...
		if (entity != null) {
			...
			if (desiredActionsQueue.size() != 0) {
				...
				actionsTabSheet.setSelectedTab(actionPathContainers
	...
	//		
	// AbstractComponentContainer selectedTabContent =
	// (AbstractComponentContainer) tabSheet.getSelectedTab();
	// UserAction ua = (UserAction) selectedTabContent.getData();
	// if (entity != null) {
	// Deque<String> desiredActionsQueue =
	// entity.getState().getDesiredActionTabsQueue();
	// /*
	// * if a desired action exists, it will be set afterwards, otherwise allow
	// the first action from the
	...
	//
	// List<String> currentUserTypes = getCurrentUserTypes(entity, app);
	// Map<String, ComponentContainer> actionPathContainers = new
	// HashMap<String, ComponentContainer>();
	// List<UserAction> actionsList = new
	// ArrayList<UserAction>(ual.getActions().values());
	// for (UserAction cua : actionsList) {
	//
	// /* display only the actions that the user is allowed to see */
	// if (!cua.allowRead(currentUserTypes)) {
