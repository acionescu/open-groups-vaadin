...
public class OpenGroupsUriHandler implements URIHandler, FragmentChangedListener, Serializable {
    ...
    private Map<String, FragmentHandler> uriHandlers = new HashMap<String, FragmentHandler>();
    ...
    private void initUriHandlers() {
	uriHandlers.put("img", new FragmentHandler() {
	...
	/* an image request */
//	if ("img".equals(au.getHandlerUri())) {
//	    if (au.getParamUri() == null) {
//		app.showNotification("file.not.found", Notification.TYPE_ERROR_MESSAGE);
//		return null;
//	    }
//	    String basePath = app.getContext().getBaseDirectory().getAbsolutePath();
//	    File file = new File(basePath + "/VAADIN/themes/" + app.getTheme() + "/" + relativeUri);
//	    FileResource fileResource = new FileResource(file, app);
//	    return fileResource.getStream();
//	}
	...
	if (app.getCurrentUri().length() > 0) {
	    logger.debug("Discarding fragment because uri present: '"+app.getCurrentUri()+"'");
	    return;
	}
	...
	logger.debug("handle fragment " + fragment + " at " + currentTime + " dif " + dif);
