...
//	}
	...
	entitiesTabSheet.addListener(new SelectedTabChangeListener() {
		...
		/*
		 * when the tab is changed, set the associated entity as selected entity, and set the tab last used
		 * container as targetComponent
		 */
		AbstractComponent container = (AbstractComponent) entitiesTabSheet.getSelectedTab();
		if (container != null) {
		    ...
		    /* set uri fragment */
		    // uriUtility.setFragment(UriFragments.SHOW_ENTITY_FRAGMENT + newEntity.getId(), false);
		    setFragmentToCurrentEntity();

		    TabContainer tabContainer = tabsForEntities.get(newEntity.getId());
		    ...
		    // System.out.println("current selected entity: " + getSelectedEntity());
		    // System.out.println("tab changed " + newEntity.getTitle());
		    // selectedEntityStack.push(newEntity);
		    // System.out.println("stack:" + selectedEntityStack.size());
		}
	    }
	});
	...
	entitiesTabSheet.addListener(new ComponentDetachListener() {
		...
		AbstractComponentContainer c = (AbstractComponentContainer) event.getDetachedComponent();
	...
	uriUtility.setFragment(UriFragments.SHOW_ENTITY_FRAGMENT + currentEntity.getId() + "/"
	...
	actionsManager.executeAction(ActionsManager.LOAD_MAIN_WINDOW, null, this, null, false);
    }

    public void refreshCurrentSelectedEntity() {
	// System.out.println("refreshing current selected entity. stack " + selectedEntityStack);
	// // if (selectedEntityStack.size() == 1) {
	// // refreshMainWindow();
	// // } else {
	// actionsManager.executeAction(ActionsManager.OPEN_AND_REFRESH_ENTITY, getSelectedEntity(), this,
	...
	actionsManager.executeAction(ActionsManager.OPEN_ENTITY_WITH_ACTIONS, entity, this, cc, false);
	if (hasErrors()) {
	    handleErrors();
	}
    }
	...
	fullyRefreshEntity(getSelectedEntity());
    }
	...
	if (tab == null) {
	    ...
	    tab = entitiesTabSheet.addTab(tabContent, shortenCaption(entity.getTitle(), 20), null);
	...
	if (tab == null) {
	    ...
	    tab = entitiesTabSheet.addTab(tabContent, shortenCaption(entity.getTitle(), 20), null);
	    tab.setClosable(closable);
	    actionsManager.executeAction(ActionsManager.OPEN_ENTITY_WITH_ACTIONS, entity, this, tabContent, false);
	...
	if (hasErrors()) {
	    handleErrors();
	    entitiesTabSheet.setSelectedTab(tabsForEntities.get(prevEntity.getId()).getContainer());
    ...
    /**
     * @param lastFragmentUpdate the lastFragmentUpdate to set
     */
