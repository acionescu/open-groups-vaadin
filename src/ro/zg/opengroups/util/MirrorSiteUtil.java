...
public class MirrorSiteUtil {
    private static final Logger logger = MasterLogManager.getLogger("MirrorSiteUtil");
	...
	if (fragment != null && !"".equals(fragment.trim())) {
	    ...
	    id = (Long) getAppConfigManager().getApplicationConfigParam(ApplicationConfigParam.DEFAULT_ENTITY_ID);
	    path="entity.list.recent.activity";
	}
	try {
    ...
    private void generatePageForIdAndPath(PrintWriter out, long id, String path, int page) {
    ...
    private void generateListFragment(PrintWriter out, EntityList list, Entity entity) {
	...
	if (state.isOpened() || (subtyesList == null && parentEntityId < 0)) {
	    ...
	    out.append(HtmlHelper.wrapAsA(getUrlForEntity(entity), entity.getTitle()));
	} else { /* leaf entity */
	    Entity parentEntity = new Entity(entity.getParentEntityId());
	    parentEntity.getState().setEntityTypeVisible(true);
	    parentEntity.getState().setDesiredActionsPath(entity.getComplexType() + "/LIST/entity.list.newest");
	    out.append(HtmlHelper.wrapAsA(getUrlForEntity(parentEntity), entity.getTitle()));
	}
	/* add tags */
	...
	int totalPages = (int)Math.ceil(totalItems/entity.getState().getItemsPerPage());
	int currentPage = entity.getState().getCurrentPageForCurrentAction();
	entity.getState().setDesiredActionsPath(entity.getState().getCurrentActionPath().substring(1));
	if(currentPage > 1) {
	...
	return baseUrl + "/" + UriFragments.SHOW_ENTITY_FRAGMENT + entity.getId() + "/"
		+ entity.getState().getCurrentPageForCurrentAction() + entity.getState().getDesiredActionsPath();

    }

    public static void main(String[] args) throws MalformedURLException {
	URL url = new URL("http://localhost:8080/open-groups#showEntity/247/1/COMMENT/LIST/entity.list.most_popular");
	String base = url.getProtocol() + "://" + url.getHost();
	if (url.getPort() > 0) {
	    base += ":" + url.getPort();
	}
