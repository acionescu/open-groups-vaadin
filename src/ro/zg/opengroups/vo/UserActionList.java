...
//    private static Map<String, String> actionTargetsDisplayNames = new LinkedHashMap<String, String>();
//
//    static {
...
//	actionTargetsDisplayNames.put("SOLUTION/LIST", "List proposed solutions");
//    }
...
//	    addAction(ua.getActionTarget(), ua);
	...
	if (index < 0) {
	    UserActionList ual = getUserActionList(actionTarget, ua.getActionPath());
	    ual.addAction(ua);
	} else {
	    String actionTargetPrefix = actionTarget.substring(0, index);
	    UserActionList ual = getUserActionList(actionTargetPrefix, actionTargetPrefix);
	    ual.addAction(actionTarget.substring(index + 1), ua);
	}
    }

    private UserActionList getUserActionList(String actionTarget, String fullActionTarget) {
	UserActionList ual = (UserActionList) actions.get(actionTarget);
	if (ual == null) {
	    ual = new UserActionList(actionTarget);
	    ual.setDisplayName(OpenGroupsResources.getMessage("action-list."+fullActionTarget));
